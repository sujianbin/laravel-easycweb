jQuery(function() {
    ...
    // 拖拽时不接受 js, txt 文件。目前只有谷歌浏览器支持此api
    ...
    function setState( val ) {
        ...
        switch ( state ) {
            ...
            case 'confirm':
                $progress.hide();
                $( '#filePicker2' ).removeClass( 'element-invisible' );
                $upload.text( '开始上传' ).addClass( 'disabled sss' ).hide();
    ...
    uploader.on( 'uploadSuccess', function( file, response) {
        if(uploader.options.fileNumLimit == fileCount){
            $("#filePicker2").hide();
        }
        ...
        if(response.code == 200){
            ...
            $( '#'+file.id ).on('click','span.cancel',function(){
